...
//@ts-ignore
const EditMenuItemVariantComponent = ({changeNavigation}) => {
    ...
    const saveChanges = async () => {
        ...
        const {success, message} = await MenuItemsBackendApiService.updateMenuItemVariant({
            ...payload,
        });
    ...
    const getAllInitialData = async () => {
        ...
        setAllVariantsData(variantsData);
    ...
    useEffect(() => {
        let selectedVariantObj: any;
        const selectedCategories: string[] = [];
        const updatedMaterials: any = {};
        const updatedMaterialsFields: any = [];

        allVariantsData.map((variant: any) => {
            if (variant.id === selectedVariant) {
                selectedVariantObj = variant;
                setSelectedMenuItemId(variant.menuItemId);
            }
        });
        ...
        const updatedFields = fields.map((field: any) => {
            if (field.fieldName === 'material') {
                // materials
                selectedVariantObj.materials.map((material: any, index: number) => {
                ...
                return {
                    ...field,
                    value: selectedVariantObj[field.fieldName]
                }
            }
        });
...
export default EditMenuItemVariantComponent;
